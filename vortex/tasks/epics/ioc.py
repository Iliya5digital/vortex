...
class IocHost(AbstractIoc):
    @task
    def run(self, ctx: Context, addr_list: List[str] = [], debug: bool = False) -> None:
        self.build(ctx)
        ...
        args = [str(binary), str(script)]
        if debug:
            args = ["gdb", "-ex", "run", "-ex", "bt", "-args", *args]
        cwd = script.parent
...
class IocCross(AbstractIoc):
    ...
    def _post_deploy(self, ctx: Context) -> None:
        ...
        for ioc_name in [path.name for path in boot_dir.iterdir()]:
            ...
            ctx.output.store(text.encode(), self.deploy_path / "iocBoot" / ioc_name / "envPaths")
